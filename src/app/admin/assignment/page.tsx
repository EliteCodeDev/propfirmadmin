...
export default function AssignmentPage() {
  ...
  const [selectedRelation, setSelectedRelation] = useState<
    ChallengeRelation | null | undefined
  >(null);
    ...
    if (selectedBalance && matchingVariation) {
      const postData = {
        ...
        product: {
          ...
          variationID:
            selectedBalance.relationBalances?.find(
              (value) => value.relationID === selectedRelation?.relationID
            )?.wooID || 999,
          name: `${selectedPlan.name} - ${selectedCategory.name} - ${
            selectedBalance.name || selectedBalance.balance
          }`,
        ...
        <div className="relative min-h-[420px] flex items-center justify-center">
          ...
          <div className="relative flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Loading challenge configuration…
            </p>
          </div>
        </div>
      </MainLayout>
      ...
      <div
        className={classNames(
          "w-8 h-8 grid place-items-center rounded-full border text-sm font-semibold",
            ...
            : "bg-white border-zinc-300 dark:bg-zinc-900 dark:border-zinc-700 text-zinc-500"
        )}
      >
          ...
          <div className="relative p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
                Choose plan, category and balance; then select the buyer.
              </p>
            </div>
            {/* Visual stepper */}
            <div className="flex items-center gap-6">
              ...
              <Step
                ...
                active={!!selectedPlan && !selectedCategory}
              />
              <div className="w-10 h-px bg-zinc-200 dark:bg-zinc-400" />
              <Step
                index={3}
                label="Balance"
                done={!!selectedBalance}
                active={!!selectedCategory && !selectedBalance}
              />
              <div className="w-10 h-px bg-zinc-200 dark:bg-zinc-400" />
              <Step
                index={4}
                label="Buyer"
                active={!!selectedBalance && !selectedUser}
                done={!!selectedUser}
              />
            </div>
          </div>
                          ...
                          >
                            <input
                              ...
                              onChange={() =>
                                ...
                                handleBalanceClick(balanceDetail.balance)
                              }
                              className="sr-only"
                            />
                            <label
                            ...
                            >
                              ...
                              {filteredEmailSuggestions.map((user, index) => (
                                <div
                                  key={user.id}
                                  onClick={() =>
                                    handleEmailSuggestionClick(user)
                                  }
                                  className={classNames(
