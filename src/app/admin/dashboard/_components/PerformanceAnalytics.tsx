...
export default function PerformanceAnalytics() {
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        if (err instanceof Error && err.message.includes("401")) {
          setError(
            "Authentication required. Please log in to view analytics data."
          );
        } else {
          setError(
  ...
  const data = useMemo(() => {
    ...
    const combinedData = Array.from(allDates)
      .map((date) => {
        const orderData = analyticsData.orderVolume.find(
          (item) => item.date === date
        );
        const userData = analyticsData.userRegistrations.find(
          (item) => item.date === date
        );
        const withdrawalData = analyticsData.withdrawalRequests.find(
          (item) => item.date === date
        );
      ...
      <div className="relative p-6 sm:p-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            <div className="p-3 bg-white/60 rounded-2xl shadow-md">
            ...
            <div>
              ...
              <div className="mt-2 flex items-center gap-4 text-sm">
                ...
                <div className="flex items-center gap-2">
                  <span className="w-8 h-0.5 bg-gray-400 rounded-full" />{" "}
                  <span className="text-gray-600 dark:text-gray-300">
                    Clients
                  </span>
                </div>
          ...
          <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-sm border transition ${
                  range === r
                    ...
                    : "bg-white/70 dark:bg-transparent text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700"
                }`}
              >
                {r}
              </button>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-sm transition ${
                range === "Custom"
                  ? "bg-indigo-600 text-white shadow"
                  : "bg-white/70 dark:bg-transparent text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700"
              }`}
            >
              Custom
            </button>
          ...
          <div className="mt-5 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/70 dark:bg-transparent p-4 flex flex-col sm:flex-row gap-3 sm:items-center">
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
              <label className="flex flex-col gap-1 text-sm">
                ...
                <input
                  ...
                  className="px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white/90 dark:bg-gray-900/30 outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </label>
              <label className="flex flex-col gap-1 text-sm">
                <span className="text-gray-600 dark:text-gray-300">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-4 py-2 rounded-2xl text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800"
              >
                Cancel
              </button>
              <button
                onClick={() => setShowCustom(false)}
                className="px-4 py-2 rounded-2xl text-white bg-gradient-to-r from-blue-500 to-indigo-600 shadow"
              >
                Apply
              </button>
            </div>
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(value: number | string, name: string) => [
                    name === "revenues" ? `$${value}` : value,
                    name === "revenues" ? "Revenues" : "Clients",
                  ]}
                />
                <Legend />
                <Area
