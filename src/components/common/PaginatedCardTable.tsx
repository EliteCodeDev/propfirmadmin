...
const renderCell = (column: ColumnConfig, row: Record<string, unknown>) => {
  ...
  switch (column.type) {
    case "link": {
      const url =
        typeof column.linkUrl === "function"
          ? column.linkUrl(value, row)
          : column.linkUrl || "#";
      return (
        <a
          href={url}
...
export default function PaginatedCardTable(props: PaginatedCardTableProps) {
  ...
  const { pageNumbers, startPage, endPage } = usePageNumbers(
    currentPage,
    totalPages
  );
            ...
            <TableHeader>
              <TableRow className="bg-gray-50 dark:bg-gray-700/30 backdrop-blur-sm border-gray-200 dark:border-gray-600/50">
                {columns.map((c) => (
                  <TableHead
                    key={c.key}
                    className={`px-3 py-3 text-left text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-gray-700 dark:text-gray-300 whitespace-nowrap ${getColumnWidth(
                      c
                    )}`}
                  >
                    {c.label}
            ...
            <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell
                    colSpan={colCount}
                    className="px-3 py-8 text-center bg-gray-50/50 dark:bg-gray-800/20"
                  >
                ...
                <TableRow>
                  <TableCell
                    colSpan={colCount}
                    className="px-3 py-12 bg-gray-50/50 dark:bg-gray-800/20"
                  >
                    <div className="flex flex-col items-center justify-center">
                      {emptyIcon}
                ...
                rows.map((row, idx) => (
                  <TableRow
                    key={idx}
                    className="hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors border-gray-200 dark:border-gray-700/30"
                  >
                    {columns.map((c) => (
                      <TableCell
                        key={c.key}
                        className={`px-3 py-3 align-middle whitespace-nowrap text-xs text-gray-900 dark:text-gray-200 ${getColumnWidth(
                          c
                        )}`}
                      >
                        <div
                  ...
                  {startPage > 1 && (
                    <>
                      <button
                        ...
                        className="px-2.5 py-1 rounded-md text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600/30 transition-colors"
                      >
                        1
                      </button>
                      {startPage > 2 && (
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-md text-xs transition-colors ${
                        currentPage === p
                          ...
                          : "text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600/30"
                      }`}
                    >
                      {p}
                    </button>
